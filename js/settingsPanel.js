...
function updateChatFontSize(state) {
    const chatContainer = document.getElementById('chat-container');
    if (chatContainer) {
        chatContainer.style.fontSize = state.fontSize + 'px';
    }

    // 프로필, 설정 저장
    if (typeof StorageManager !== 'undefined' && StorageManager) {
        const profiles = StorageManager.loadProfiles();
        StorageManager.saveProfiles(profiles, state.selectedUsers, { fontSize: state.fontSize });
    }
}
